...
def read_root(num:str):
    ...
    print(zxfbje_jhje, zxfbje_jhje_jjx, zxfbje_htje, zxfbje_htje_jxs, zxfbje_sfje, zxfbje_sfje_jxs, zxfbje_dfje,
          zxfbje_ddkjxs)
    ...
    # 7.计划巡检次数与实际巡检次数对比
    sql = "select count(*) from project_check_plans a INNER JOIN project_check_plan_items b on a.id=b.parent_id where a.project_id='%s'" % (
        project_id)
    cursor.execute(sql)
    result = cursor.fetchall()
    if result == ():
    ...
    # #劳务合同比例
    sql = "select count(*) from Subcontracts a inner join subcontract_items b on a.id=b.parent_id where approval_status='Approval' and a.project_id='%s'" % (
        project_id)
    cursor.execute(sql)
    result = cursor.fetchall()
    if result == ():
    ...
    sql = "select count(b.tax_rate) from Subcontracts a inner join subcontract_items b on a.id=b.parent_id where approval_status='Approval' and a.project_id='%s' and tax_rate =3.00 GROUP BY b.tax_rate ORDER BY tax_rate" % (
        project_id)
    cursor.execute(sql)
    result = cursor.fetchall()
    if result == ():
    ...
    # 材料合同（13%）比例
    sql = "select count(b.tax_rate) from Subcontracts a inner join subcontract_items b on a.id=b.parent_id where approval_status='Approval' and a.project_id='%s' and tax_rate =13.00 GROUP BY b.tax_rate ORDER BY tax_rate" % (
        project_id)
    cursor.execute(sql)
    result = cursor.fetchall()
    if result == ():
    ...
    # 累计开票金额
    sql = "select sum(amount) from Purchase_Invoices where approval_status='Approval' and project_id='%s'" % (
        project_id)
    cursor.execute(sql)
    result = cursor.fetchall()
    if result == ():
    ...
    # 质保金到期日
    sql = "select b.date_plan from Contracts a inner join contract_collection_plans b on a.id=b.parent_id and a.name='%s' and b.phase='zhibaojin'" % (
        name)
    cursor.execute(sql)
    result = cursor.fetchall()
    if result == ():
